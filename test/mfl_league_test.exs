defmodule MFLLeagueTest do
  ...
  test "free_agents/3", %{league: league, year: year, bypass: bypass} do
    Bypass.expect_once bypass, fn conn ->
      ...
      assert "TYPE=freeAgents&L=#{league}&JSON=1" == conn.query_string
      assert "GET" == conn.method
      Plug.Conn.resp(conn, 200, ~s<{"freeAgents":{"leagueUnit":{"unit":"LEAGUE","player":[{"contractYear":"","contractStatus":"","id":"11682","contractInfo":"","salary":"0.00"},{"contractYear":"","contractStatus":"","id":"12681","contractInfo":"","salary":"0.00"}]}},"version":"1.0","encoding":"utf-8"}>)
    end
    ...
    assert League.rosters(year, league) == [
                                             %{"player" => [
                                                             ...
                                                             %{"contractYear" => "35",
                                                               ...
                                                               "status" => "ROSTER",
                                                               "id" => "9902",
                                                               "contractInfo" => "2016:  Extended for three years (originally 3L-2017) to 5L-2020",
                                                               "salary" => "2.00"
                                                             }
                                                           ],
                                               "id" => "0001"
  ...
  test "salary_adjustments/3", %{league: league, year: year, bypass: bypass} do
    Bypass.expect_once bypass, fn conn ->
      ...
      assert "GET" == conn.method
      Plug.Conn.resp(conn, 200, ~s<{"version":"1.0","salaryAdjustments":{"salaryAdjustment":[{"amount":"2.00","timestamp":"1373254913","franchise_id":"0001","id":"0","description":"kept of wright's salary after trade"},{"amount":"13.00","timestamp":"1447997124","franchise_id":"0003","id":"99","description":"Dropped Anderson in-season"}]},"encoding":"utf-8"}>)
    end
